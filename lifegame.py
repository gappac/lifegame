...
for y in range(0, ROWS): # ステージをランダムに初期化
    data.append([(randint(0, 9) <= 3) for x in range(0, COLS)])
...
cv = Canvas(root, width = root_width, height = root_height) # キャンバスを作成
cv.pack()
cv.create_line(0, 0, root_width, 0, width=10)
...
def draw_stage():
    ...
    for y in range(0, ROWS):
        for x in range(0, COLS):
            ...
            x1, y1 = [x * CW, y * CW]
            cv.create_rectangle(x1+1, y1+1, x1 + CW-1, y1 + CW-1,
                fill="red", width=0) # 生きているセルを描画
# 300ミリ秒ごとに世代を進める --- (*6)
def game_loop():
    next_turn() # 世代を進める
    draw_stage() # ステージを描画
    root.after(1000, game_loop) # 指定時間後に再度描画
game_loop() # ゲームループを実行
